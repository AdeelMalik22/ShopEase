...
Existing PostgreSQL Schema — Since the database is user-provided:
...
AUTH_USER_MODEL must be set FIRST — Set AUTH_USER_MODEL = 'accounts.CustomUser' in settings.py before the very first migration. Changing it later breaks everything.

Dual Template Backends — Django Admin uses its own template engine. Jinja2 must be added as a second backend, NOT a replacement, to avoid breaking the admin panel:
...
PDF Library — Use ReportLab only. WeasyPrint requires heavy system-level dependencies (Cairo, Pango, GLib) that complicate deployment. ReportLab is pure Python.

Async Tasks — Use Celery + Redis for deadline notifications, bulk payroll, and all email sending.

Email Backend — Password reset and notifications require SMTP. Configure SendGrid (production) or Gmail (dev) before testing auth flows.
